...
On exact verified game builds 1.004 and 1.005, startup automatically attempts the reviewed extended transport capability for charts with at most 8192 source rows and 8192 compact native events. Playable authority remains unavailable until every build-specific helper, caller, signature, layout, and callback-vtable check passes and the reserve hook installs successfully. Unknown, mismatched, or partially validated builds remain native-512. Rows may contain a monotone, a chord, or both; monotone and chord sides carry independent NoteType 0–6 and DotType 0–1 values. Native NoteTypes 5 and 6 are accepted from the recovered duration mapping but remain unqualified in-game. Strength and authored camera state remain unchanged by this authoring feature. Automatically generated MIDI rows are ungrouped, while chord IgnoreSound slots and explicitly authored groups use the exact native ownership/linking path described below.
...
Native chart events own subarrays and referenced state, and links point into the stable event allocation. Generalized admission therefore requires the exact build-specific 1.004 or 1.005 helper/signature specification, canonical compact/link plan, parser GroupIndex suppression, transaction-local ownership journal, and bounded `E<=8192`. Independent side NoteType/DotType values remain limited to the reviewed authoring domain; camera/strength changes and builds without a complete reviewed specification remain unauthorized.
