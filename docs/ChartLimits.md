...
On exact verified game builds 1.004 and 1.005, startup automatically attempts the reviewed extended transport capability for charts with at most 8192 source rows and 8192 compact native events. Playable authority remains unavailable until every build-specific helper, caller, signature, layout, and callback-vtable check passes and the reserve hook installs successfully. Unknown, mismatched, or partially validated builds remain native-512. Rows may contain a monotone, a chord, or both; monotone and chord sides carry independent reviewed NoteType 0–4 and DotType 0–1 values. Strength and authored camera state remain unchanged by this authoring feature. Automatically generated MIDI rows are ungrouped, while chord IgnoreSound slots and explicitly authored groups use the exact native ownership/linking path described below.
