...
Focused Development session `20260902T175102Z-a761305b6d59` qualified this exact restricted path on verified game build 1.005 at checkpoint `88f1f99`. The native reserve changed from 512 to 513 once, the prefix, tail, and callback validated, count 513 committed, results counted all 513 events, natural completion succeeded, and a subsequent ordinary 290-row song restored chart and audio behavior. This evidence does not generalize to arbitrary row counts, mixed event shapes, another executable build, or a packaged release artifact.
